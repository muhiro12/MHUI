...
private struct MHKeyValueRowValidationPreview: View {
    var body: some View {
        VStack(alignment: .leading, spacing: MHTheme.standard.spacing.section) {
            MHKeyValueFallbackRowsPreview()
            MHKeyValueAlignedRowsPreview()
        }
    }
}

private struct MHKeyValueFallbackRowsPreview: View {
    var body: some View {
        MHGroupedRows {
            LabeledContent(
            ...
            LabeledContent(
                "Validation target",
                value: """
                    Long labels, long values, and accessibility type should remain
                    practical at 375pt and 320pt widths.
                    """
            )
            .labeledContentStyle(.mhKeyValue)
        }
...
private struct MHKeyValueAlignedRowsPreview: View {
    var body: some View {
        MHGroupedRows {
