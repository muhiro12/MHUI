...
struct MHDesignMetricsTests {
    ...
    func standard_metrics_define_shared_layout_defaults() {
        ...
        #expect(metrics.cornerRadius.surface == 8)
        #if os(watchOS)
        #expect(metrics.layout.readableContentWidth == 320)
        #expect(metrics.layout.compactWidthThreshold == 304)
        #expect(metrics.layout.screen.contentInsetHorizontal == 16)
        #expect(metrics.layout.screen.contentInsetVertical == 16)
        #expect(metrics.layout.screen.contentSpacing == 16)
        #expect(metrics.layout.screen.compactContentInsetHorizontal == 16)
        #expect(metrics.layout.screen.compactContentInsetVertical == 16)
        #expect(metrics.layout.screen.compactContentSpacing == 16)
        #expect(metrics.layout.surface.insetHorizontal == 16)
        #expect(metrics.layout.surface.insetVertical == 16)
        ...
        #expect(metrics.layout.screen.contentInsetVertical == 40)
        ...
        #endif
        #if os(macOS)
        #expect(metrics.layout.control.minimumTouchTarget == 32)
        #else
        #expect(metrics.layout.control.minimumTouchTarget == 48)
        #endif
    }
